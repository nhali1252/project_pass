...
def get_hardware_id():
    try:
        identifiers = []
        try: identifiers.append(hex(uuid.getnode()))
        except: pass
        if platform.system() == 'Windows':
            try:
                import winreg
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Cryptography")
                identifiers.append(winreg.QueryValueEx(key, "MachineGuid")[0])
            except: pass
        ...
        if not identifiers: identifiers.append(str(uuid.getnode()))
        return hashlib.sha256('|'.join(identifiers).encode()).hexdigest()
    except: return hashlib.md5(str(uuid.getnode()).encode()).hexdigest()

# ================= ADB ডিভাইস ডিটেক্টর =================
def get_adb_devices():
    """ADB devices কমান্ড রান করে ডিভাইস লিস্ট রিটার্ন করে"""
    try:
...
def verify_with_server(password):
    ...
    try:
        response = requests.post(f"{SERVER_URL}/api/client/verify", json={
            "pc_name": get_pc_name(), "hardware_id": get_hardware_id(), "password": password
        }, timeout=15)
        data = response.json()
        if data.get("success"):
            ...
            return True
        else: print(Fore.RED + f"❌ {data.get('message')}"); return False
    except requests.exceptions.ConnectionError:
        print(Fore.RED + "❌ Cannot connect to server! Check internet."); return False
    except Exception as e:
        print(Fore.RED + f"❌ Error: {str(e)}"); return False

def status_and_device_polling():
    """একই থ্রেডে স্টেটাস চেক এবং ডিভাইস রিপোর্ট করা"""
    global LICENSE_VALID, STOP_POLLING, SESSION_TOKEN
    while not STOP_POLLING:
        time.sleep(10)  # ১০ সেকেন্ড পর পর
        try:
            # 1. স্টেটাস চেক
            if SESSION_TOKEN:
                status_resp = requests.post(f"{SERVER_URL}/api/client/status", json={"session_token": SESSION_TOKEN}, timeout=5)
                status_data = status_resp.json()
                if not status_data.get("active"):
                    print(Fore.RED + "\n⛔️ Admin has deactivated your license! Closing software...")
                    LICENSE_VALID = False; STOP_POLLING = True; os._exit(0)
                
                # 2. ডিভাইস রিপোর্ট করা
                devices = get_adb_devices()
                requests.post(f"{SERVER_URL}/api/client/device_info", json={"session_token": SESSION_TOKEN, "devices": devices}, timeout=5)
        except: pass
...
def main():
    ...
    password = input(Fore.WHITE + "\n🔑 Enter the Global Password: ").strip()
    if not verify_with_server(password):
        input(Fore.RED + "\n❌ Press Enter to exit..."); sys.exit(1)

    # পোলিং থ্রেড স্টার্ট
    polling_thread = threading.Thread(target=status_and_device_polling, daemon=True)
    ...
    while LICENSE_VALID:
        ...
        if choice.upper() == 'Q':
            print(Fore.YELLOW + "\nExiting software..."); STOP_POLLING = True; break
        else:
            commands = {
                '1': 'adb push preload.so /data/local/tmp/preload.so', '2': 'adb shell chmod 755 /data/local/tmp/preload.so',
                '3': 'adb shell "LD_PRELOAD=/data/local/tmp/preload.so id"', '4': 'adb push su /data/local/tmp/su',
                '5': 'adb shell chmod 755 /data/local/tmp/su', '6': 'adb shell "/data/local/tmp/su -c \'id\'"',
                '7': 'adb shell "/data/local/tmp/su -c \'getenforce\'"', '8': 'adb push abl.elf /data/local/tmp/abl',
